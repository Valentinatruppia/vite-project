  ...
  <script>
    ...
    async function fetchDatiAirbag() {
      try {
        ...
        const Airbag_ine = array.filter((i) => i["Airbag"] == "Inesistente");
        const Airbag_esploso = array.filter((e) => e["Airbag"] == "Esploso");
        const Airbag_inesploso = array.filter(
          (n) => n["Airbag"] == "Inesploso"
        );
        y_Airbag = [
          Airbag_ine.length,
          Airbag_esploso.length,
          Airbag_inesploso.length,
        ];
    ...
    async function fetchDatiConducenti() {
      try {
        ...
        const conducenti = data.filter(
          (persona) => persona["TipoPersona"] === "Conducente"
        );
        const percentualeConducenti = (conducenti.length / data.length) * 100;

        plotIstogrammaConducenti(
          percentualeConducenti,
          "myChartConducenti",
          "Distribution of Drivers and Passenger"
        );
      } catch (error) {
        console.error(`Impossibile recuperare i dati: ${error}`);
      }
    ...
    function plotIstogrammaConducenti(
      percentualeConducenti,
      chartId,
      titleText
    ) {
    ...
    async function fetchDatiConducentiPast() {
      try {
        ...
        const conducentiPast = data.filter(
          (persona) => persona["TipoPersona"] === "Conducente"
        );
        const percentualeConducentiPast =
          (conducentiPast.length / data.length) * 100;

        plotIstogrammaConducentiPast(
          percentualeConducentiPast,
          "myChartConducentiPast",
          "Distribution of Drivers and Passenger"
        );
      } catch (error) {
        console.error(`Impossibile recuperare i dati: ${error}`);
    ...
    // Aggiungi questa funzione per visualizzare il grafico della percentuale di conducenti
    function plotIstogrammaConducentiPast(
      percentualeConducenti,
      chartId,
      titleText
    ) {
    ...
    async function fetchDatiProgressivo() {
      try {
        ...
        data.forEach((persona) => {
          ...
          progressivoCounts[progressivo] =
            (progressivoCounts[progressivo] || 0) + 1;
        });

        // Calcola la percentuale per ogni valore di "Progressivo"
        const totalEntries = data.length;
        const percentualiProgressivo = Object.keys(progressivoCounts).map(
          (progressivo) => ({
            progressivo,
        ...
        PlotIstogrammaProgressivo(
          percentualiProgressivo,
          "myChartProgressivo",
          "Distribution of Progressivo(2022)"
        );
      } catch (error) {
        console.error(`Impossibile recuperare i dati: ${error}`);
      }
    ...
    async function fetchDatiProgressivoPast() {
      try {
        ...
        // Chiamata alla funzione per visualizzare il grafico a torta
        plotIstogrammaProgressivoPast(
          percentualiProgressivo,
          "myChartProgressivoPast",
          "Distribution of Progressivo(2008)"
        );
      } catch (error) {
        console.error(`Impossibile recuperare i dati: ${error}`);
      }
    ...
    async function fetchDatiLesioni() {
      try {
        ...
        const percentualiLesioni = Object.entries(lesioniCount).map(
          ([tipoLesione, count]) => ({
            tipoLesione,
        ...
        plotTortaLesioni(
          percentualiLesioni,
          "myChartLesioni",
          "Distribution of Injury Types(2022)"
        );
      } catch (error) {
        console.error(`Impossibile recuperare i dati: ${error}`);
      }
    ...
    async function fetchDatiLesioniPast() {
      try {
        ...
        // Chiamata alla funzione per visualizzare il grafico delle lesioni a torta
        plotTortaLesioniPast(
          percentualiLesioni,
          "myChartLesioniPast",
          "Distribution of Injury Types (2008)"
        );
      } catch (error) {
        console.error(`Impossibile recuperare i dati: ${error}`);
      }
    ...
    async function fetchDatiCinturaCasco() {
      try {
        ...
        const totalPersonas = data.length;
        const percentualiCinturaCasco = Object.entries(cinturaCascoCount).map(
          ([cinturaCasco, count]) => ({
            cinturaCasco,
            percentuale: (count / totalPersonas) * 100,
          })
        ...
        plotTortaCinturaCasco(
          percentualiCinturaCasco,
          "myChartCinturaCasco",
          "Distribution of Seatbelt/Helmet Usage(2022)"
        );
      } catch (error) {
        console.error(`Impossibile recuperare i dati: ${error}`);
      }
    ...
    function plotTortaCinturaCasco(
      percentualiCinturaCasco,
      chartId,
      titleText
    ) {
    ...
    async function fetchDatiCinturaCascoPast() {
      try {
        ...
        plotTortaCinturaCascoPast(
          percentualiCinturaCasco,
          "myChartCinturaCascoPast",
          "Distribution of Seatbelt/Helmet Usage (2008)"
        );
      } catch (error) {
        console.error(`Impossibile recuperare i dati: ${error}`);
      }
    ...
    function plotTortaCinturaCascoPast(
      percentualiCinturaCasco,
      chartId,
      titleText
    ) {
